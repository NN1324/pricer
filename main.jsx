    ...
    try {
      ...
      const text = await callClaude(apiKey, [{
        role:'user', content:[
          ...
          { type:'text', text:`Extract all products and prices from this supplier invoice or price list.
Return ONLY a valid JSON array. No markdown, no explanation, no code fences.
Each object: name (string), price (number), priceIncludesIva (boolean), category (string e.g. "Refrescos","Cerveza","Vino","Spirits","Alimentación","Snacks"), supplier (string from header else "Desconocido"), unit (string e.g. "caja/24","botella","barril","kg"), iva (number: 0/4/10/21 using Spanish rules: alcohol=21, refrescos/food=10, basics=4).
Example: [{"name":"Coca-Cola 33cl","price":8.50,"priceIncludesIva":false,"category":"Refrescos","supplier":"Makro","unit":"caja/24","iva":10}]` }
        ]
      }])
  ...
  const handleAsk = async () => {
    ...
    try {
      const answer = await callClaude(apiKey, [{ role:'user', content:q }],
